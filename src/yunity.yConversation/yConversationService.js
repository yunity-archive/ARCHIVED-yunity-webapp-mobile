...
export default class YConversation {
   ...
   */
  get(id) {
    return this.yAPI.apiCall(`/conversations/${id}`)
  ...
  getChatForUser(userID) {
    return this.yAPI.apiCall(`/conversations-by-user/${userID}`)
      .then((response) => new Conversation(this, response.data));
  }
  ...
  createChatForUser(userID, message) {
    if (typeof message === 'string')
      message = { content: message };
    return this.yAPI.apiCall({
      uri: `/conversations-by-user/${userID}`,
      data: { message }
    }).then((response) => new Conversation(this, response.data));
  }
  ...
  addParticipants(id, participants) {
    return this.yAPI.apiCall({
      uri: `/conversations/${id}/participants`,
      data: { participants }
    });
  }
